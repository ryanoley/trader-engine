...
public class TraderApplication implements Application {
    ...
    private void executionReport(Message message, SessionID sessionID) throws FieldNotFound {
        ...
        if (ordStatus == OrdStatus.REJECTED) {
        	...
        	// 1. When there are errors in order. Replicate with BRK/B order. Wrong suffix
        	// 2. Getting here when canceling Unacknowledged orders. The broker message is: Cancelled by Exchange
        	orderBook.orderRejected(orderID);
        	return;
        } else if (ordStatus == OrdStatus.CANCELED || ordStatus == OrdStatus.DONE_FOR_DAY) {
        	// 1. Cancels generated by us 
        	// 2. Seems like rejects coming here as well. Replicate with large market orders
